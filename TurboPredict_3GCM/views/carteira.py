...
def render(loader=None):
    ...
    with col_esq:
        ...
        st.plotly_chart(fig_vol, use_container_width=True, key="bar")
    ...
    with col_dir:
        ...
        st.plotly_chart(fig_hm, use_container_width=True, key= "hm")
    ...
    st.plotly_chart(fig_esp, use_container_width=True, key="esp")

    # ── Alertas Preditivos — Média Móvel e Z-Score ───────────────────────────
    # Seção adicionada para monitoramento preditivo da inadimplência:
    # - Semáforo de tendência (↑→↓) comparando último mês vs anterior
    # - Média móvel de 7 períodos para suavizar variações
    # - Z-score para detectar spikes anômalos
    ...
    st.plotly_chart(fig_mm, use_container_width=True, key="mm")
