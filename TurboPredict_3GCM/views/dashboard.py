...
def render(loader=None):
    ...
    <div class="turbo-header">
        <div>
            ...
            <div class="turbo-subtitle">Dashboard de Risco · Análise de Recebíveis FIAP/Núclea 2026</div>
        </div>
    </div>
    """, unsafe_allow_html=True)

    # ── Barra de busca MELHORADA ─────────────────────────────────────────────
    # ALTERAÇÃO 1: removido o busca[:8] — agora busca em qualquer parte do ID
    # MOTIVO: antes só buscava pelos primeiros 8 caracteres, limitando muito os resultados
    busca = st.text_input(
        "🔍  Busca Turbo",
        placeholder="Buscar por ID do beneficiário (parcial ou completo)...",
        label_visibility="collapsed",
    )

    if busca and len(busca) >= 3:
        # ALTERAÇÃO 2: busca em qualquer parte do ID, não só no início
        resultados = df[df["id_beneficiario"].str.contains(busca, case=False, na=False)]

        if not resultados.empty:
            st.success(f"⚡ {len(resultados)} cedente(s) encontrado(s) para '{busca}'")

            # ALTERAÇÃO 3: se encontrar mais de um, mostra lista pra escolher
            # MOTIVO: antes mostrava só o primeiro resultado sem opção de escolha
            if len(resultados) > 1:
                opcoes = {
                    row["id_beneficiario"][:40] + "...": idx
                ...
                _render_cedente_card(resultados.loc[idx_escolhido])
            else:
                # ALTERAÇÃO 4: se encontrar só um, mostra direto
                _render_cedente_card(resultados.iloc[0])
            return
        else:
            st.warning(f"Nenhum cedente encontrado para '{busca}'.")

    # ALTERAÇÃO 5: aviso mínimo de 3 caracteres para evitar buscas vazias
    elif busca and len(busca) < 3:
